...
from D4RL_SE2.d4rl.pointmaze.gridcraft.grid_spec_se2 import EMPTY, WALL, TILES, START, RENDER_DICT, REWARD, GOAL
...
ACT_DICT = {
    ...
    ACT_RIGHT: [+0.1, 0, 0],
    ACT_DOWN: [0, +0.1, 0],
    ACT_CW: [0, 0, +0.1], # 1 means rotating for THETA_GRID_RES rad
    ACT_CCW: [0, 0, -0.1]
}
ACT_TO_STR = {
    ACT_NOOP: 'NOOP',
...
# class GridEnvSE2(OfflineEnv, gym.Env):
class GridEnvSE2(gym.Env):
    def __init__(self, maze_str=O_MAZE,
                 tiles=TILES,
                 rew_fn=None,
                 teps=0.0, 
                 max_timesteps=None,
    ...
    #     if h5path is None:
    #         h5path = self.dataset_path
    #     return super(GridEnvSE2, self).get_dataset(h5path)

    def get_dataset(self, h5path=None):
        if h5path is None:
            h5path = self.dataset_path
        data_dict = {}
        print(f'^&^& load dataset from {h5path}')
        with h5py.File(h5path, 'r') as dataset_file:
            for k in tqdm(get_keys(dataset_file), desc="load datafile"):
                try:  # first try loading as an array
                    data_dict[k] = dataset_file[k][:]
                    # print(f'^&^& dataset shape: {data_dict[k].shape}')
    ...
    def action_space(self):
        ...
        space = gym.spaces.Box(0, 1, shape=(3,))
        return space

    @property
    def observation_space(self):
        print('execute observation_space')
        dO = len(self.gs)
        print(dO)
        return gym.spaces.Box(0,1,shape=(6,))
