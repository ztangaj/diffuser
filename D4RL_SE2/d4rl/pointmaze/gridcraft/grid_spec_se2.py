...
class GridSpecSE2(object):
    ...
    # def get_neighbors(self, k, xytheta=False):
    #     """ Return values of up, down, left, and right tiles """
    #     if not xytheta:
    #         k = self.idx_to_xytheta(k)
    ...
    # def get_value(self, k, xytheta=False):
    #     """ Return values of up, down, left, and right tiles """
    #     if not xytheta:
    #         k = self.idx_to_xytheta(k)
    #     return self[k]
    ...
    def idx_to_xytheta(self, idx):
        xy_idx = idx % (self.__w * self.__h)
        xy = self.idx_to_xy(xy_idx)
        theta = np.floor(idx/(self.__w * self.__h)).astype(np.int32)
        xytheta = np.concatenate((xy, np.array([theta])))
        return xytheta

    def xytheta_to_idx(self, key):
        shape = np.array(key).shape
        if len(shape) == 1:
            return (key[0] + key[1]*self.__w) + key[2] * (self.__w * self.__h)
        # elif len(shape) == 2:
        #     return key[:,0] + key[:,1]*self.__w
        else:
            raise NotImplementedError()
