## Notes for COMP5214 Project

* For dataset generation, please use scripts `D4RL/scripts/generation/generate_maze2d_se2_datasets.py`. See [wiki from D4RL](https://github.com/Farama-Foundation/D4RL/wiki/Dataset-Reproducibility-Guide).
* I use environment `GridEnvSE2` instead of `MazeEnv`. The `GridEnvSE2` is defined in `D4RL/d4rl/pointmaze/gridcraft/grid_env_se2.py`. The grid spec is modified accordingly to a SE2 version.
* The waypoint controller is also modified to a SE2 version in `D4RL/d4rl/pointmaze/waypoint_controller_se2.py`.
